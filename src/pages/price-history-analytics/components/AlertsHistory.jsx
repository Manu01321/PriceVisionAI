...
const AlertsHistory = ({ productId }) => {
  ...
  const filterOptions = [
    { value: 'all', label: 'All Alerts', count: alertsHistory?.length },
    {
      value: 'triggered',
      label: 'Triggered',
      count: alertsHistory?.filter((a) => a?.status === 'triggered')?.length
    },
    {
      value: 'active',
      label: 'Active',
      count: alertsHistory?.filter((a) => a?.status === 'active')?.length
    },
    {
      value: 'missed',
      label: 'Missed',
      count: alertsHistory?.filter((a) => a?.status === 'missed')?.length
    }
  ];
          ...
          sortedAlerts?.map((alert) => (
            ...
            >
              <div className="flex items-start space-x-4">
                {/* Alert Icon */}
                <div
                  className={`w-10 h-10 rounded-lg bg-muted flex items-center justify-center ${getAlertColor(alert?.type, alert?.status)}`}
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                    ...
                    {alert?.timeframe && (
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-sm text-foreground">Within {alert?.timeframe}</span>
                      </div>
                    )}
                  </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-warning">
                {Math.round(
                  alertsHistory?.reduce((sum, a) => sum + a?.confidence, 0) / alertsHistory?.length
                )}
                %
              </div>
