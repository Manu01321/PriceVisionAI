...
class ImageSearchService {
    ...
    try {
      ...
      if (error instanceof ApiError) {
        if (error.isNetworkError) {
          throw new Error(
            'Unable to connect to image search service. Please check your internet connection.'
          );
  ...
  validateImageFile(file) {
    ...
    if (file.size > this.maxImageSize) {
      throw new Error(
        `Image file is too large. Maximum size is ${this.maxImageSize / (1024 * 1024)}MB.`
      );
    }
  ...
  processImageSearchResults(response, imageData) {
    ...
    return {
      ...
      results: results.map((product) =>
        this.enhanceImageSearchResult(product, response.extractedInfo)
      ),
  ...
  calculateVisualSimilarity(product, aiAnalysis) {
    ...
    if (aiAnalysis?.features && aiAnalysis.features.length > 0) {
      ...
      const featureMatches = aiAnalysis.features.filter(
        (feature) =>
          titleLower.includes(feature.toLowerCase()) || descLower.includes(feature.toLowerCase())
      );
  ...
  checkCategoryMatch(product, aiAnalysis) {
    ...
    for (const [category, related] of Object.entries(categoryRelations)) {
      if (
        (productCategory.includes(category) && related.includes(extractedCategory)) ||
  ...
  findFeatureMatches(product, aiAnalysis) {
    ...
    return features
      .filter((feature) => searchText.includes(feature.toLowerCase()))
      .map((feature) => ({
        feature,
        found: true,
