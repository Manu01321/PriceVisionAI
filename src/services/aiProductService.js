    ...
    const response = await fetch(this.openaiEndpoint, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: 'user', content: `Search for products in India: "${query}"${options.category ? ` in category: ${options.category}` : ''}${options.priceRange ? ` with price range: ₹${options.priceRange.min}-₹${options.priceRange.max}` : ''}` }
        ],
        temperature: 0.7,
        max_tokens: 2500,
...
Return ONLY the JSON, no other text.`;
    ...
    const response = await fetch(`${this.geminiEndpoint}?key=${GEMINI_API_KEY}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { 
            role: 'user', 
            content: [
              { type: 'text', text: 'Identify this product and find matching products in the market.' },
              { 
                type: 'image_url', 
                image_url: { 
...
Return 5-10 real products matching the image. ONLY return JSON. Prices must be in INR and every product must include a non-empty image URL.`;
  ...
  normalizeProduct(product, index) {
    ...
    const parsedOriginal = Math.round(
