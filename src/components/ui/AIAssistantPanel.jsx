...
const AIAssistantPanel = ({ 
  ...
  const initialMessages = [
    {
      id: 1,
      type: 'assistant',
      content: "Hi! I'm your AI shopping assistant. I can help you find the best deals, compare products, and track prices. What are you looking for today?",
  ...
  const handleSendMessage = async () => {
    ...
    try {
      if (!aiProductService.isAvailable()) {
        throw new Error('No AI API key configured. Please add VITE_OPENAI_API_KEY or VITE_GEMINI_API_KEY in your .env file.');
      }
      ...
      if (products.length > 0) {
        const top = products.slice(0, 3).map(
          p => `• ${p.name} — ₹${Number(p.currentPrice || 0).toLocaleString('en-IN')}${p.discount ? ` (${p.discount}% off)` : ''}`
        ).join('\n');
      ...
      setMessages(prev => [...prev, {
        ...
        timestamp: new Date(),
        suggestions: ['Show price history', 'Find similar products', 'Set price alert', 'Compare alternatives']
      }]);
    } catch (err) {
      setMessages(prev => [...prev, {
        id: Date.now() + 1,
  ...
  return (
    <>
      ...
      />
      {/* Panel */}
      <div className={`fixed right-0 top-0 h-full w-full max-w-md bg-surface border-l border-border shadow-elevated z-300 lg:relative lg:w-80 lg:shadow-none lg:border-l-0 lg:border border-border lg:rounded-lg ${className}`}>
        {/* Header */}
            ...
            >
              <div className={`max-w-[80%] ${message?.type === 'user' ? 'order-2' : 'order-1'}`}>
                ...
                <div className={`p-3 rounded-lg ${
                  message?.type === 'user' ?'bg-primary text-primary-foreground ml-8' :'bg-muted text-foreground mr-8'
                }`}>
