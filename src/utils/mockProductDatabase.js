...
// Nokia Phones
{
  ...
  confidence: 92,
  dealUrgency: 75,
  aiInsight: "Perfect basic phone for calls and texts. Long battery life and durable design make it ideal for backup or elderly users.",
...
},
{
  ...
  imageAlt: "Nokia 105 feature phone in blue showing traditional T9 keypad and monochrome display",
  currentPrice: 24.99,
  originalPrice: 29.99,
  discount: 17,
  confidence: 89,
  dealUrgency: 68,
  aiInsight: "Ultra-affordable basic phone with weeks of battery life. Perfect for emergencies or digital detox.",
...
},
{
  ...
  confidence: 94,
  dealUrgency: 82,
  aiInsight: "Eco-friendly smartphone with user-repairable design. Great value 5G phone with clean Android experience.",
...
// Apple Products
{
  ...
  confidence: 95,
  dealUrgency: 85,
  aiInsight: "Premium flagship with titanium build and advanced camera system. Best-in-class performance and display quality.",
...
},
{
  ...
  confidence: 93,
  dealUrgency: 78,
  aiInsight: "Solid mid-range iPhone with excellent cameras and reliable performance. Great value after price drop.",
...
// Samsung Products
{
  ...
  confidence: 92,
  dealUrgency: 78,
  aiInsight: "Flagship Android with S Pen and exceptional zoom capabilities. Perfect for productivity and photography.",
...
},
{
  ...
  imageAlt: "Samsung Galaxy A54 smartphone in violet purple showing modern design with triple camera",
  currentPrice: 349,
  originalPrice: 449,
  discount: 22,
  confidence: 88,
  dealUrgency: 86,
  aiInsight: "Excellent mid-range phone with great cameras and 5G. Outstanding value with premium features.",
...
// Google Pixel
{
  ...
  confidence: 88,
  dealUrgency: 92,
  aiInsight: "Best computational photography with Google AI features. Significant discount makes this a hot deal.",
...
// OnePlus
{
  ...
  confidence: 85,
  dealUrgency: 65,
  aiInsight: "Flagship performance with ultra-fast charging. Great value in the premium segment with OxygenOS.",
...
// Xiaomi
{
  ...
  confidence: 82,
  dealUrgency: 58,
  aiInsight: "Photography powerhouse with Leica partnership. Professional-grade camera system in flagship body.",
...
// Nothing Phone
{
  ...
  confidence: 79,
  dealUrgency: 72,
  aiInsight: "Unique transparent design with Glyph lighting system. Distinctive aesthetics meet solid mid-range performance.",
...
// Budget/Feature Phones
{
  ...
  confidence: 76,
  dealUrgency: 84,
  aiInsight: "Ultra-budget Android smartphone with decent performance for basic tasks. Great first smartphone option.",
...
// Tablets
{
  ...
  confidence: 91,
  dealUrgency: 73,
  aiInsight: "Powerful tablet with M1 chip and Apple Pencil support. Perfect for productivity and creative work.",
...
// Laptops
{
  ...
  confidence: 94,
  dealUrgency: 88,
  aiInsight: "Exceptional performance laptop with all-day battery life. M2 chip delivers pro-level power in ultra-portable design.",
...
// Audio Devices
{
  ...
  confidence: 93,
  dealUrgency: 87,
  aiInsight: "Industry-leading noise cancellation with spatial audio. Significant discount on premium wireless earbuds.",
...
// Smart Watches
{
  ...
  imageAlt: "Apple Watch Series 9 in midnight aluminum showing health tracking interface and sport band",
  currentPrice: 379,
  originalPrice: 429,
  discount: 12,
  confidence: 89,
  dealUrgency: 71,
  aiInsight: "Advanced health monitoring with ECG and blood oxygen sensors. Latest watchOS features and all-day battery.",
...
// Gaming
{
  ...
  imageAlt: "Sony PlayStation 5 console in white and black showing modern design with DualSense controller",
  currentPrice: 499,
  originalPrice: 499,
  discount: 0,
  confidence: 96,
  dealUrgency: 45,
  aiInsight: "Next-gen gaming console with 4K gaming and ray tracing. High demand item with limited availability.",
...
// Accessories
{
  ...
  imageAlt: "Anker PowerCore portable battery charger in black showing compact design with USB ports",
  currentPrice: 19.99,
  originalPrice: 29.99,
  discount: 33,
  confidence: 87,
  dealUrgency: 91,
  aiInsight: "Compact high-capacity power bank with fast charging. Excellent deal on essential mobile accessory.",
...
export function searchProducts(query, filters = {}) {
  ...
  if (filters?.priceRange?.min) {
    filteredProducts = filteredProducts?.filter((p) => p?.currentPrice >= parseFloat(filters?.priceRange?.min));
  }
  if (filters?.priceRange?.max) {
    filteredProducts = filteredProducts?.filter((p) => p?.currentPrice <= parseFloat(filters?.priceRange?.max));
  }
  ...
  if (filters?.confidenceThreshold) {
    filteredProducts = filteredProducts?.filter((p) => p?.confidence >= filters?.confidenceThreshold);
  }
  if (filters?.dealQuality && filters?.dealQuality !== 'all') {
    const thresholds = { hot: 80, good: 60, fair: 40 };
    filteredProducts = filteredProducts?.filter((p) => p?.dealUrgency >= thresholds?.[filters?.dealQuality]);
  }
...
export function getProductsByCategory(category) {
  return mockProductDatabase?.filter((product) =>
...
export function getFeaturedDeals(limit = 6) {
  return mockProductDatabase?.filter((product) => product?.discount >= 15 || product?.dealUrgency >= 80)?.sort((a, b) => b?.dealUrgency - a?.dealUrgency)?.slice(0, limit);
}

// Get trending products (high ratings and reviews)
export function getTrendingProducts(limit = 6) {
  return mockProductDatabase?.filter((product) => product?.rating >= 4.2 && product?.reviews >= 300)?.sort((a, b) => b?.rating * Math.log(b?.reviews) - a?.rating * Math.log(a?.reviews))?.slice(0, limit);
}
