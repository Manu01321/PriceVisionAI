...
class AmazonScraper extends BaseScraper {
  ...
  async extractRating(element) {
    const ratingSelectors = ['.a-icon-alt', '.a-star-alt'];
    ...
    for (const selector of ratingSelectors) {
      try {
        ...
        if (ratingElement) {
          const ratingText =
            (await ratingElement.getAttribute('alt')) || (await ratingElement.textContent());
          if (ratingText) {
  ...
  async extractImage(element) {
    const imageSelectors = ['.s-image', 'img[data-image-index]', '.rush-component img'];
  ...
  async extractProductUrl(element) {
    const linkSelectors = ['h2 a', '.a-link-normal', 'a[href*="/dp/"]'];
  ...
  async extractProductRating(page) {
    const ratingSelectors = ['.a-icon-alt', '.reviewCountTextLinkedHistogram .a-link-normal'];
  ...
  async extractProductReviewCount(page) {
    const reviewSelectors = ['#acrCustomerReviewText', '.reviewCountTextLinkedHistogram'];
  ...
  async extractProductAvailability(page) {
    const availabilitySelectors = ['#availability span', '.a-color-success', '.a-color-price'];

    return await this.getTextWithFallback(page, availabilitySelectors, 'unknown');
  }

  async extractProductBrand(page) {
    const brandSelectors = ['#bylineInfo', '.a-row .a-link-normal', '.po-brand .po-break-word'];
