...
class ProductSearchService {
  ...
  async searchByText(query, options = {}) {
    ...
    try {
      ...
      if (this.cacheClient && this.cacheClient.isReady) {
        this.cacheClient
          .set(cacheKey, JSON.stringify(processedResults), {
            EX: this.cacheTTL
          })
          .catch((err) => {
      ...
      this.logger.info(
        `Search completed in ${responseTime}ms with ${processedResults.length} results`
      );
  ...
  async searchWithOpenAI(query, options = {}) {
    try {
        ...
        [
          {
            role: 'system',
            content:
              'You are an e-commerce product data generator. Return only valid JSON array, no markdown.'
          },
      ...
      return products.map((p, i) => ({
        ...
        source: {
          site: p.source || 'amazon',
          name: p.source === 'flipkart' ? 'Flipkart' : 'Amazon India'
        },
        imageUrl: p.imageUrl || '',
  ...
  calculateEnhancedRelevanceScore(product, query) {
    ...
    queryWords.forEach((word) => {
      ...
      if (wordIndex !== -1) {
        ...
        const exactMatch =
          title.includes(` ${word} `) || title.startsWith(`${word} `) || title.endsWith(` ${word}`);
  ...
  calculateRatingScore(product) {
    ...
    const reviewBonus = product.reviewCount > 100 ? 10 : product.reviewCount > 10 ? 5 : 0;
