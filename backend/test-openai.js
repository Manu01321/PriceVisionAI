...
async function testFallbackScenario() {
  ...
  try {
    ...
    status.keys.forEach((key, i) => {
      logger.info(`Key ${key.index}: Active=${key.active}, Fails=${key.failCount}, Used=${key.lastUsed ? '✅' : '❌'}`);
    });
...
async function testImageAnalysis() {
  ...
  try {
    ...
    logger.info('Note: Image analysis requires valid image URLs and may have different rate limits');
  }
}
